...
| Сетевые ошибки (`ConnectionError`, `OSError`) | экспоненциальный backoff до 60 с. При `retry_forever` (по умолчанию) попытки не ограничены `TGV_MAX_RETRIES`, потолок — `max_attempts` (200) |
| Telegram принял запрос истории и молчит | `asyncio.wait_for` 180 с → `FetchStalled`. Чтение истории прекращается, уже поставленные в очередь файлы докачиваются, задача → `failed` с текстом «Telegram не ответил…». Иначе задача висела бы вечно: у Telethon своего таймаута нет |
| Загрузка файла зависла | таймаут, пропорциональный размеру (от 300 с, из расчёта 20 КБ/с, не больше часа), затем повтор. Без него зависший воркер забивал очередь и морозил весь экспорт |
| Прогресса нет 5 / 15 минут | сторожевой таймер пишет предупреждение, затем снимает задачу с объяснением. Паузы FloodWait и ручная пауза не считаются простоем |
| Файл оборвался на середине | докачка с последнего байта через `iter_download` (`resume_partial`), смещение выравнивается по 4096 |
| Файлы, не скачавшиеся за основной проход | `_sweep_failures()` перечитывает сообщения (свежие `file_reference`) и добирает остаток; проходы идут, пока дают результат |
| Постоянные ошибки (`MediaEmptyError`, `FileIdInvalidError`, …) | повтор бессмысленен — файл сразу помечается `failed` |
| Отмена не доходит до зависшей корутины | `cancel_job` даёт 8 с на кооперативную остановку, затем снимает `asyncio.Task` принудительно и чистит реестр. Без этого задача навсегда оставалась «выполняющейся» и её нельзя было удалить |
